...
const baseCreateRenderer = (renderOptionsDom) => { //渲染vnode=>render,渲染时所到的api
	//元素操作方法
	const {
		//创建元素
		createElement,
	...
	const processText = (n1, n2, container) => {
		if (n1 === null) {//创建文本，渲染到页面
			insertElement((n2.el = createText(n2.children)), container)
		} else {
	...
	const patchKeyedChildren = (c1, c2, container, anchor = null) => {
		...
		while (i <= e1 && i <= e2) {  // 从左向右比较
			const n1 = c1[i];
			const n2 = c2[i];
			if (isSameVnode(n1, n2)) { // 相同就 patch
		...
		if (i > e1) { // 说明有新增 
			...
			for (let i = toBePatched - 1; i >= 0; i--) {
				...
				if (newIndexToOldIndexMap[i] == 0) { // 这是一个新元素 直接创建插入到 当前元素的下一个即可
