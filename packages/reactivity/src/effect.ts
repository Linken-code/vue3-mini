//定义effect 收集依赖，更新视图
import { isArray, isInteger, IsDEV, isMap, extend } from '@vue/shared'
...
export class ReactiveEffect<T = any> {
  ...
  constructor(public fn: () => T, public scheduler = null, scope = null) {
...
export const Track = (target, type, key) => {
  ...
  //3.2
  // 开发环境和非开发环境
  const eventInfo = IsDEV ? { effect: activeEffect, target, type, key } : undefined
...
//触发更新
export const Trigger = (target: object, type, key?: unknown, newValue?: unknown, oldValue?: unknown) => {
  // 从依赖管理中心中获取依赖
  ...
  // 判断开发环境，传入eventInfo
  const eventInfo = IsDEV ? { target, type, key, newValue, oldValue } : undefined
